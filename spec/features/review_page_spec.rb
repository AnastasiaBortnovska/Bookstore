RSpec.describe BookReview do
    let(:book_review) { BookReview.new }
    let!(:review) { create(:review, :approved).decorate }
    ...
    shared_examples 'review informations' do
      ...
      it { expect(book_review).to have_create_data(text: review.slash_date_format) }
    ...
    describe 'when user log in' do
      ...
      it 'failurefully fill form' do
        ...
        expect(book_review).to_not have_div_success(text: I18n.t('message.success.review.create'))
      end
    ...
    describe 'when user log out' do
      ...
      it { expect(book_review).to_not have_review_form }
      include_examples 'review informations'
    end
end
